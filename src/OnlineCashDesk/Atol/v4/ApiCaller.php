...
class ApiCaller implements \OnlineCashDesk\ApiCaller
{
    ...
    public function sendReceipt(array $receiptData)
    {
        ...
        $apiResponse = $this->makeRequest($receiptData['group_code'] . '/' . $receiptData['document_type'] . '?token=' . $token, $receiptData);
    ...
    public function checkReceiptStatus(array $receiptData, array $lastResponseData)
    {
        ...
        $apiResponse = $this->makeRequest($receiptData['group_code'] . '/report/' . $lastResponseData['uuid'] . '?token=' . $token, null, 'GET');
    ...
    private function getToken()
    {
        ...
        if (!$this->token) {
            ...
            if (!empty($response['error'])) {
                $this->lastError = (!empty($response['error']['text']) ? $response['error']['text'] : 'Неизвестная ошибка при попытке получения токена')
                    . (' [код: ' . (!empty($response['error']['code']) ? $response['error']['code'] : 'n/a') . ']')
                    . ' (TOKEN)';
            } else {
